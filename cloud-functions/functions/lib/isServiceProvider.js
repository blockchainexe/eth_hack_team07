...
module.exports = firebase.functions.https.onRequest((req, res) => {
  const data = abi.simpleEncode("isServiceProvider(address):(bool)", req.body.address);
  const param = {
  ...
  return client.geth.request('eth_call', [param, 'latest'], (err, response) => {
    if (err) {
      console.error('Error getting service provider: ', err)
    ...
    const decodedParameter = abi.rawDecode(['bool'], new Buffer(response.result.slice(2), 'hex'));
