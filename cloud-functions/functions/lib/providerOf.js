...
module.exports = firebase.functions.https.onRequest((req, res) => {
  const data = abi.simpleEncode("providerOf(address):(string)", req.body.address);
  const param = {
  ...
  return client.geth.request('eth_call', [param, 'latest'], (err, response) => {
    ...
    const decodedParameter = abi.rawDecode(['string'], new Buffer(response.result.slice(2), 'hex'));
